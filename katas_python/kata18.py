...
def nbmonths(startold, startnew, savingpermonth, percentloss):
    month = saving = 0
    while True:
        # start of the month
        # if startold >= startnew: return [month, startold - startnew]
        if startold >= startnew: return [month, startold - startnew] if month == 0 else [month, startnew - startold]
        month += 1
        if month % 2 == 0: percentloss += 0.5

        # end of the month
        saving += savingpermonth    
        startold -= startold * percentloss / 100
        startnew -= startnew * percentloss / 100
        if saving + startold >= startnew: return [month, round(saving + startold - startnew)]
